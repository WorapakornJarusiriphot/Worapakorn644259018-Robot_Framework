...
Fill Birthdate
    [Arguments]    ${date}
    # แปลง string วันเดือนปีเกิดเป็น list ของตัวอักษร
    ${length}=    Evaluate    len(${date})
    ${chars}=    Create List
    FOR    ${index}    IN RANGE    ${length}
        Append To List    ${chars}    ${date}[${index}]
    END
    
    # โฟกัสไปที่ฟิลด์วันเดือนปีเกิด
    Set Focus To Element    css=input[placeholder="MM/DD/YYYY"]
    
    # พิมพ์ตัวเลขวันเดือนปีเกิดทีละตัว
    FOR    ${char}    IN    @{chars}
        Press Keys    css=input[placeholder="MM/DD/YYYY"]    ${char}
        Sleep    1s
    END
