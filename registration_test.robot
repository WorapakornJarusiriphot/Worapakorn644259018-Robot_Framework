...
*** Test Cases ***
TC 2001 ทดสอบการสมัครสมาชิกด้วยข้อมูลที่ถูกต้อง
    ...
    Sleep    1s
    Wait Until Element Is Visible    //div[contains(@class, 'MuiAlert-message') and contains(text(), 'สมัครสมาชิกสำเร็จ!')]    timeout=30s
    Capture Page Screenshot    screenshots/TC-2/TC20001-success_${TIMESTAMP}.png

TC 2002 ทดสอบการสมัครสมาชิกโดยไม่กรอกข้อมูล
    [Documentation]    ทดสอบการสมัครสมาชิกโดยไม่กรอกข้อมูล
    [Tags]    registration    negative
    ${TIMESTAMP}=    Get Current Date    result_format=${TIME_FORMAT}
    Click Button    css=button[type="submit"]
    Wait Until Element Is Visible    xpath=//p[contains(text(),'กรุณากรอกชื่อจริง')]    timeout=30s
    Wait Until Element Is Visible    xpath=//p[contains(text(),'กรุณากรอกนามสกุล')]    timeout=30s
    Wait Until Element Is Visible    xpath=//p[contains(text(),'กรุณากรอกชื่อผู้ใช้')]    timeout=30s
    Wait Until Element Is Visible    xpath=//p[contains(text(),'กรุณากรอกหมายเลขโทรศัพท์')]    timeout=30s
    Wait Until Element Is Visible    xpath=//p[contains(text(),'กรุณากรอกอีเมล')]    timeout=30s
    Wait Until Element Is Visible    xpath=//p[contains(text(),'กรุณากรอกรหัสผ่าน')]    timeout=30s
    Wait Until Element Is Visible    xpath=//p[contains(text(),'กรุณาเลือกวันเกิด')]    timeout=30s
    Wait Until Element Is Visible    xpath=//p[contains(text(),'กรุณาเลือกเพศ')]    timeout=30s
    Capture Page Screenshot    screenshots/TC-2/TC20002-fail_${TIMESTAMP}.png

TC 2003 ทดสอบการสมัครสมาชิกด้วยอีเมลที่ไม่ถูกต้อง
    ...
    Press Keys    css=input[placeholder="MM/DD/YYYY"]    \\13
    Sleep    1s
    Wait Until Element Is Visible    xpath=//p[contains(text(),'กรุณากรอกอีเมลให้ถูกต้อง')]    timeout=30s
    Capture Page Screenshot    screenshots/TC-2/TC20003-fail_${TIMESTAMP}.png

TC 2004 ทดสอบการสมัครสมาชิกด้วย Username ที่ซ้ำกับในฐานข้อมูล
    ...
    Press Keys    css=input[placeholder="MM/DD/YYYY"]    \\13
    Sleep    15s
    Wait Until Element Is Visible    xpath=//p[contains(text(),'Username นี้มีคนใช้แล้ว')]    timeout=30s
    Capture Page Screenshot    screenshots/TC-2/TC20004-fail_${TIMESTAMP}.png

TC 2005 ตรวจสอบการสมัครสมาชิกด้วย Password ที่ไม่ถูกต้อง
    ...
    Press Keys    css=input[placeholder="MM/DD/YYYY"]    \\13
    Sleep    15s
    Wait Until Element Is Visible    xpath=//p[contains(text(),'รหัสผ่านต้องมีอย่างน้อย 8 ตัวอักษร')]    timeout=30s
    Capture Page Screenshot    screenshots/TC-2/TC20005-fail_${TIMESTAMP}.png

TC 2006 ตรวจสอบการสมัครสมาชิกด้วยหมายเลขโทรศัพท์ที่ไม่ถูกต้อง
    ...
    Press Keys    css=input[placeholder="MM/DD/YYYY"]    \\13
    Sleep    15s
    Wait Until Element Is Visible    xpath=//p[contains(text(),'เบอร์โทรศัพท์ต้องเป็นหมายเลขเบอร์โทรศัพท์จริงๆเท่านั้น')]    timeout=30s
    Capture Page Screenshot    screenshots/TC-2/TC20006-fail_${TIMESTAMP}.png

TC 2007 ตรวจสอบการสมัครสมาชิกด้วย Phone Number ที่ไม่ถูกต้อง
...
TC 2008 ตรวจสอบการสมัครสมาชิกด้วยวันเกิดที่ไม่ถูกต้อง
    ...
    Press Keys    css=input[placeholder="MM/DD/YYYY"]    \\13
    Sleep    15s
    Wait Until Element Is Visible    xpath=//p[contains(text(),'วันเกิดต้องไม่ต่ำกว่า 10 ปีจากวันปัจจุบัน')]    timeout=60s
