...
Register With Valid Data
    ...
    Click Element    css=button[aria-label="Choose date"]
    # เพิ่มการรอให้ DatePicker แสดงผล
    Wait Until Element Is Visible    css=.MuiPickersCalendar-transitionContainer
    # เลือกวันที่ใน DatePicker
    Click Element    xpath=//button[contains(@aria-label, "Jan 1, 1990")]
...
Register With Duplicate Email Or Username
    ...
    Click Element    css=button[aria-label="Choose date"]
    Wait Until Element Is Visible    css=.MuiPickersCalendar-transitionContainer
    Click Element    xpath=//button[contains(@aria-label, "May 13, 2024")]
    Click Element    css=input[type='radio'][value='ชาย']
    Click Button    css=button[type="submit"]
