...
*** Test Cases ***
TC 10001 ทดสอบการเข้าร่วม โพสต์นัดเล่น ที่หน้า Home
    ...
    Wait Until Element Is Visible    //div[contains(@class, 'MuiAlert-message') and contains(text(), 'คุณได้ทำการเข้าร่วมโพสต์นัดเล่นสำเร็จแล้ว')]    timeout=30s
    # Run Keyword And Continue On Failure    Execute JavaScript    document.getElementById("Sign-Out").click();
    Sleep    2s
    Capture Page Screenshot    screenshots/TC-10/TC10001-success_${TIMESTAMP}.png
